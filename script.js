...
const S = {
  pane: 'stream', filter: 'All', year: 'All', q: '',
...
const matches = f => {
  ...
  if (S.year !== 'All' && YEAR_OF(f) !== S.year) return false;
  if (!S.q) return true;
...
function renderChrome(shown) {
  ...
  const counts = {
    ...
    research: C.projects.length + C.past.length,
...
/* ── panes ──────────────────────────────── */
function paneStream() {
  const ALL = streamItems(), rows = ALL.filter(matches).map(decorate), lead = leadPaper();
  ...
  ${C.site.showFigures === false ? '' : `<div style="display:flex;flex-wrap:wrap;gap:52px;padding:28px 0 24px">${figures.map(k => `<div><p class="num serif" style="font:300 38px/1 'Source Serif 4',serif;color:var(--ink);margin:0 0 8px;letter-spacing:-.022em">${esc(k.n)}</p><p class="lab-s">${esc(k.label)}</p></div>`).join('')}</div>`}
  ...
  <div style="border-top:2px solid var(--teal);padding:20px 0 26px;display:grid;grid-template-columns:1fr 348px;gap:36px;align-items:start">
    <div>
  ...
  <div style="border-top:1px solid var(--line);padding:16px 0 4px">
  ...
  <div style="position:sticky;top:0;background:var(--surface);padding:16px 0 14px;display:flex;flex-wrap:wrap;gap:22px;align-items:center;z-index:5;border-top:1px solid var(--line);margin-top:18px">
  ...
  ${rows.map(f => `<div class="row" style="display:grid;grid-template-columns:92px 1fr 152px;gap:22px;padding:20px 12px;margin:0 -12px;border-top:1px solid var(--hair);align-items:start">
    <p class="num" style="font:400 10.5px Ubuntu,sans-serif;letter-spacing:.14em;text-transform:uppercase;color:var(--muted);margin:5px 0 0">${esc(f.date)}</p>
    <div style="min-width:0">
      <p style="display:flex;align-items:center;gap:7px;font:500 9.5px Ubuntu,sans-serif;letter-spacing:.18em;text-transform:uppercase;color:var(--muted);margin:0 0 7px"><span class="dot" style="background:${f.color}"></span>${esc(f.type)}<span style="color:var(--line2)">·</span>${esc(f.format)}</p>
      <a href="${esc(f.href)}" style="font:400 19px/1.35 'Source Serif 4',serif;color:var(--ink);display:block;max-width:62ch;hanging-punctuation:first last;text-wrap:pretty">${esc(f.title)}</a>
      <p style="font:300 12.5px/1.6 Ubuntu,sans-serif;color:var(--muted);margin:5px 0 0;max-width:70ch">${esc(f.venue)}</p>
      ${(f.actions || []).length ? `<div style="display:flex;gap:18px;margin-top:10px">${f.actions.map(a => `<a class="link" href="${esc(a.href)}">${esc(a.label)}</a>`).join('')}</div>` : ''}
    </div>
    <p style="font:400 12px/1.55 'Source Serif 4',serif;font-style:italic;color:var(--muted);margin:4px 0 0;text-align:right">${esc(f.note)}</p>
  </div>`).join('')}
  ...
  <p class="num" style="font:300 11.5px Ubuntu,sans-serif;letter-spacing:.06em;color:var(--muted);margin:22px 0 0;border-top:1px solid var(--hair);padding-top:16px">${rows.length} of ${ALL.length} items · papers come from the ORCID record, refreshed every Monday morning</p>

  <div style="border-top:1px solid var(--line);padding:16px 0 26px;margin-top:26px">
...
function paneResearch() {
  ...
  ${C.projects.map(p => `<div style="border-top:1px solid var(--line);padding:20px 0 24px;display:grid;grid-template-columns:150px 1fr;gap:26px">
    <div>
    ...
    <div>
      ...
      <p style="font:300 11px/1.55 Ubuntu,sans-serif;letter-spacing:.05em;color:var(--muted);margin:0">With ${esc(p.co)}</p>
    </div></div>`).join('')}
  <h3 class="hd" style="margin:32px 0 16px;border-top:1px solid var(--line);padding-top:18px">Earlier work</h3>
  <div style="columns:2;column-gap:44px">
    ${C.past.map(p => `<p style="font:400 14.5px/1.5 'Source Serif 4',serif;color:var(--ink);margin:0 0 16px;break-inside:avoid">${esc(p.title)}<br><span style="font:300 10.5px Ubuntu,sans-serif;letter-spacing:.14em;text-transform:uppercase;color:var(--muted)">${esc(p.funding)}</span></p>`).join('')}
  </div>
  <div style="margin-top:14px;padding-top:18px;border-top:1px solid var(--line)">
    <div style="display:flex;flex-wrap:wrap;align-items:baseline;gap:16px;margin-bottom:14px">
      ...
      <p style="font:300 11.5px Ubuntu,sans-serif;letter-spacing:.06em;color:var(--muted);margin:0">Across the four active studies. A filled mark means more than one shared study.</p>
    </div>
    <div style="display:flex;flex-wrap:wrap;gap:10px 22px">
      ${collab.map(c => `<p style="display:flex;align-items:center;gap:7px;font:400 14.5px 'Source Serif 4',serif;color:var(--ink);margin:0"><span class="dot" style="width:6px;height:6px;background:${c.dot};box-shadow:inset 0 0 0 1px var(--lav)"></span>${esc(c.name)}</p>`).join('')}
    </div>
  </div>
</div>`;
}
...
function panePeople() {
  return `<div data-reveal="1" style="padding-top:30px">
  <p class="quote">${esc(C.about.peopleQuote)}</p>
  <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(150px,1fr));gap:22px 20px">
    ${C.team.map(m => `<div>
      <div style="aspect-ratio:4/5;background-color:var(--tile);background-image:url('${esc(m.photo)}');background-size:cover;background-position:center top"></div>
      <p style="font:400 14.5px/1.3 'Source Serif 4',serif;color:var(--ink);margin:10px 0 3px">${esc(m.name)}</p>
      <p style="font:300 10.5px/1.45 Ubuntu,sans-serif;letter-spacing:.05em;color:var(--muted);margin:0">${esc(m.role)}</p></div>`).join('')}
  </div>
...
function paneAbout() {
  ...
  <div>
    ...
    <p style="font:300 11.5px/1.7 Ubuntu,sans-serif;color:var(--muted);margin:18px 0 0;max-width:78ch">${esc(a.funders)}</p>
  </div>
</div>`;
}
...
function wire() {
  ...
  document.addEventListener('click', e => {
    const t = e.target.closest('[data-pane],[data-year],[data-filter],[data-hit],[data-ask],#reset,#gprev,#gnext,#pesc');
    if (!t) return;
    ...
    if (t.id === 'reset') { S.filter = 'All'; S.year = 'All'; S.q = ''; return render(); }
...
Promise.all([
  ...
  $('#invNote').textContent = C.contact.invitationsNote;
  wire();
  render();
}).catch(err => {
