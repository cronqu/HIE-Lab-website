...
Promise.all([
  ...
  const setRail = (sel, val, html) => { const el = $(sel); if (el && val) el.innerHTML = html ? html(val) : esc(val); };
  const brk = s => esc(s).replace(/\n/g, '<br>');
  setRail('#railName', C.site.name, brk);
  setRail('#railCreds', [C.site.credentials, C.site.pronouns].filter(Boolean).join(' \u00a0·\u00a0 '));
  setRail('#railRole', [C.site.title, C.site.place].filter(Boolean).join('\n'), brk);
  setRail('#railTagline', C.site.tagline);
  if (C.site.name) document.title = C.site.name + (C.site.credentials ? ', ' + C.site.credentials : '');
  const inv = $('#invNote');
